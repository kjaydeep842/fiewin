...
class AdminParityController extends Controller
{
    ...
    {
        $game = Game::where('code', 'parity')->firstOrFail();

        // Ensure auto-settlement of recent 15 periods up to current time (180s interval)
        $interval = 180;
        $nowTs = time();
        ...
        $override = Cache::get('override_parity');

        return view('admin.games.parity', compact(
            'game',
            'todayBetsCount',
            'todayBetsTotal',
            'todayWinsTotal',
    ...
    public function updateRTP(Request $request)
    {
        ...
        return back()->with('success', "Parity (3-Min) Winning Chance / RTP updated to {$request->rtp_percentage}%!");
    }
    ...
    public function updateLimits(Request $request)
    {
        ...
        return back()->with('success', "Parity (3-Min) Min/Max bet limits updated successfully!");
    }
    ...
    public function setOverride(Request $request)
    {
        ...
        if ($request->filled('manual_number')) {
            Cache::put('override_parity', (int)$request->manual_number, 600);
            return back()->with('success', "Parity (3-Min) manual override set to number {$request->manual_number}!");
        } else {
            Cache::forget('override_parity');
            return back()->with('success', "Parity (3-Min) manual override cleared! Automatic RTP active.");
        }
    }
    ...
    public function toggleActive()
    {
        ...
        return back()->with('success', "Parity (3-Min) status changed to " . ($game->is_active ? 'Active' : 'Disabled') . ".");
    }
}
