/**
 * Jet Game Manager (Dedicated Jet Flight Manager)
 */
class JetGameManager {
    constructor() {
        this.renderer = null;
        this.pollTimer = null;
        this.animFrame = null;
    ...
    init() {
        this.renderer = new JetCanvasRenderer('jetCanvas');
        this.bindEvents();
        this.wsManager = new FiewinWebSocketManager('jet', window.USER_ID || null);
                ...
                if (this.serverState && this.renderer) {
                    ...
                    if (status === 'FLYING') {
                        if (!this.flightStartMs) {
                            this.flightStartMs = Date.now() - Math.max(0, (mult - 1.00) / 0.35 * 1000);
                        }
                        const elapsedSec = Math.max(0, (Date.now() - this.flightStartMs) / 1000);
                        mult = 1.00 + elapsedSec * 0.35;
                        const multDisplay = document.getElementById('jetMultiplierText');
    ...
    bindEvents() {
        const betBtn = document.getElementById('betJetBtn');
        const cashoutBtn = document.getElementById('cashoutJetBtn');
        const halfBtn = document.getElementById('btnHalfBetJet');
        const doubleBtn = document.getElementById('btnDoubleBetJet');
        const autoInput = document.getElementById('jetAutoCashoutInput');
        ...
        if (halfBtn) {
            halfBtn.addEventListener('click', () => {
                const input = document.getElementById('jetBetAmount');
                input.value = Math.max(10, Math.floor(parseFloat(input.value) / 2));
            });
        }
        ...
        if (doubleBtn) {
            doubleBtn.addEventListener('click', () => {
                const input = document.getElementById('jetBetAmount');
                input.value = Math.min(50000, Math.floor(parseFloat(input.value) * 2));
            });
        }

        // Auto Cashout Chip Event Listeners
        document.querySelectorAll('.jet-auto-chip').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                const val = btn.getAttribute('data-val');
                if (autoInput) autoInput.value = val;
                const statusTxt = document.getElementById('autoCashoutStatusText');
        ...
        if (autoInput) {
            autoInput.addEventListener('input', () => {
                const statusTxt = document.getElementById('autoCashoutStatusText');
                if (statusTxt) statusTxt.textContent = autoInput.value ? autoInput.value + 'x' : 'OFF';
            });
        }
    }
    ...
    async fetchServerState() {
        try {
            const response = await fetch('/games/jet/state', {
            ...
            console.error('Jet State Sync Error:', e);
        }
    }
    ...
    updateUI(state) {
        ...
        const multDisplay = document.getElementById('jetMultiplierText');
        const statusBadge = document.getElementById('jetStatusBadge');
        ...
        if (multDisplay) {
            ...
            if (roundStatus === 'CRASHED') {
                ...
                multDisplay.className = 'display-2 fw-black text-warning';
            }
        }
        ...
        if (statusBadge) {
            if (roundStatus === 'BETTING_OPEN') {
                ...
                statusBadge.innerHTML = `<span class="badge bg-primary px-3 py-2 fs-6">NEXT ROUND: ${secsStr}s</span>`;
            } else if (roundStatus === 'FLYING') {
                statusBadge.innerHTML = `<span class="badge bg-success px-3 py-2 fs-6">JET IN FLIGHT</span>`;
            } else {
                const secsStr = String(Math.max(0, parseInt(secondsRemaining) || 0)).padStart(2, '0');
                statusBadge.innerHTML = `<span class="badge bg-danger px-3 py-2 fs-6">CRASHED @ ${crashMult}x • NEXT ROUND: ${secsStr}s</span>`;
            }
        }
    ...
    updateActiveBetCard(bet, currentMultStr) {
        const card = document.getElementById('jetActiveBetCard');
        if (!card) return;

        if (bet.status === 'flying') {
            card.classList.remove('d-none');
            const roundIdEl = document.getElementById('jetActiveRoundId');
            const stakeEl = document.getElementById('jetActiveStake');
            const profitEl = document.getElementById('jetActiveLiveProfit');
            const payoutEl = document.getElementById('jetActivePayout');
    ...
    toggleBetControls(status) {
        ...
        if (this.userBet && this.userBet.status === 'flying') {
            ...
            if (betBtn) {
                betBtn.classList.remove('d-none');
                betBtn.className = 'btn btn-success btn-lg w-100 rounded-pill fw-bold shadow py-3';
                betBtn.disabled = false;
                betBtn.innerHTML = '<i class="bi bi-airplane-fill me-2"></i>PLACE JET BET';
            }
        }
    }

    renderHistory(history) {
        const container = document.getElementById('jetHistoryPills');
        if (!container) return;
    ...
    renderMyOrders(orders) {
        const container = document.getElementById('jetMyOrdersList');
        if (!container) return;
        if (orders.length === 0) {
            container.innerHTML = '<div class="text-muted text-center py-3">No Jet orders placed today.</div>';
            return;
        }
    ...
    showToast(message, type = 'success', customTitle = null) {
        ...
        if (type === 'cashout') {
            ...
            bgClass = 'bg-success text-white';
            icon = 'bi-airplane-fill text-white';
        }
    ...
    async placeBet() {
        ...
        try {
            ...
            const res = await fetch('/games/jet/bet', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                ...
                this.showToast(data.message || 'Bet placed successfully! Waiting for flight...', 'success', 'BET PLACED');
                if (data.new_balance) {
                    const userBalDisplay = document.getElementById('userWalletBalance');
    ...
    async cashOut() {
        ...
        try {
            const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
            const res = await fetch('/games/jet/cashout', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrf,
            ...
            console.error('Jet Cashout Error:', e);
            this.showToast('Cashout processing error', 'danger');
            this.toggleBetControls(this.serverState?.round?.status || 'BETTING_OPEN');
        }
    }
...
window.JetGameManager = JetGameManager;
